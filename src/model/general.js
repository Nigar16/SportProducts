...
const addProduct=(product_title,category_title,amount)=>{
    ...
    const  category_id=DB.query(`SELECT * FROM categories WHERE category_title = ? `,[category_title],async(err,result)=>{
        ...
        return 1;
            ...
            const query=`INSERT INTO products (product_title,category_id,category_title,amount) VALUES(?,?,?,?)`;
            const filter = [product_title, category_id[0].category_id, category_title, amount];
            DB.query(query, filter, async (err, result) => {
                if (err) {
                    throw err;
